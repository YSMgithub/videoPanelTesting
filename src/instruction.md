...
Настройка wi-fi закончена. Теперь необходимо посмотреть какой адрес панель получила от роутера через WIFI. Для этого переходим во вкладку Сеть->основные и в всплывающем списке тип сети выбираем wifi. Ниже должен отобразиться айпи адрес полученный панелью по вайфай, запомним его. С этим адресом будет выполнять все действия далее.
